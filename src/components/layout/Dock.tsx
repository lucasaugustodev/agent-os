...
import { Grid3x3, Terminal, FolderOpen, Settings, Zap, Globe, Activity } from 'lucide-react';
...
const DOCK_ITEMS = [
  ...
  { id: 'finder', icon: FolderOpen, label: 'Files' },
  { id: 'pm2', icon: Activity, label: 'PM2' },
...
export function Dock() {
  ...
  function handleDockClick(appId: string) {
    ...
    if (topInstance) bringToForeground(topInstance.instanceId);
  }
    ...
    >
      ...
      {DOCK_ITEMS.map((item) => {
        ...
        const isFocused = instances.some(
          (i) => i.appId === item.id && i.instanceId === foregroundInstanceId
        );
